...
function AdminAnalytics({ analytics, period, onPeriod }: { analytics: ReturnType<typeof getSalesAnalytics>; period: ReportPeriod; onPeriod: (period: ReportPeriod) => void }) {
  ...
  return <section className="analytics-page">
    ...
    <div className="analytics-columns"><section className="analytics-card trend-card"><div className="analytics-card-heading"><div><h3>Tren omzet</h3><p>Per hari pada periode terpilih</p></div><span className="chart-legend"><i /> Omzet</span></div><div className="trend-chart">{analytics.trend.map((point, index) => <div className="trend-column" key={point.label}><div className="trend-bar-wrap"><div className="trend-bar" style={{ height: `${Math.max(point.revenue ? (point.revenue / maxRevenue) * 100 : 0, point.revenue ? 8 : 2)}%` }} title={`${point.label}: ${formatRupiah(point.revenue)}`} /></div><span>{period === 'monthly' && index % 5 !== 0 ? '' : point.label}</span></div>)}</div>{analytics.revenue === 0 && <div className="chart-empty">Belum ada penjualan selesai pada periode ini.</div>}</section><section className="analytics-card customer-card"><div className="analytics-card-heading"><div><h3>Pelanggan</h3><p>Berbasis nomor WhatsApp</p></div><span className="customer-bubble">{analytics.uniqueCustomers}</span></div><div className="customer-total"><strong>{analytics.uniqueCustomers}</strong><span>pelanggan unik</span></div><div className="customer-rows"><div><span>Pelanggan baru</span><b>{analytics.newCustomers}</b></div><div><span>Kembali memesan</span><b>{analytics.returningCustomers}</b></div><div><span>Order tanpa WA</span><b>{analytics.guestOrders}</b></div></div></section></div>
